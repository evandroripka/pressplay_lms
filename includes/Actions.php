...
class MLB_LMS_Actions
{
    public static function init()
    {
        ...
        add_action('admin_post_mlb_lms_enroll_continue', [__CLASS__, 'handle_enroll_continue']);
        add_action('admin_post_nopriv_mlb_lms_enroll_continue', [__CLASS__, 'handle_enroll_continue']);
    ...
    public static function handle_enroll()
    {
        ...
        if (!is_user_logged_in()) {
            ...
            $continue_url = add_query_arg([
                'action'    => 'mlb_lms_enroll_continue',
                'course_id' => $course_id,
                '_wpnonce'  => wp_create_nonce('mlb_lms_enroll_continue_' . $course_id),
    ...
    public static function handle_enroll_continue()
    {
        ...
        if (!isset($_GET['_wpnonce']) || !wp_verify_nonce($_GET['_wpnonce'], 'mlb_lms_enroll_continue_' . $course_id)) {
            wp_die('Nonce inválido.');
        }
    ...
    private static function do_enroll_and_redirect_to_checkout($user_id, $course_id)
    {
        ...
        // matrícula pending
        MLB_LMS_Enrollments::get_or_create_pending((int)$user_id, (int)$course_id, 'woocommerce');

        $product_id = MLB_LMS_Enrollments::get_course_product_id((int)$course_id);
