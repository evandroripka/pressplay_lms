...
class MLB_LMS_Vimeo
{
    const OPT_TOKEN = 'mlb_lms_vimeo_token';
    ...
    public static function get_token()
    {
        ...
        if (class_exists('MLB_LMS_Settings')) {
            $t = MLB_LMS_Settings::get('vimeo_token', '');
    ...
    public static function api_get($path)
    {
        ...
        if (!$token) return new WP_Error('mlb_vimeo_no_token', 'Vimeo token não configurado.');
        ...
        if ($code >= 400) {
            ...
            return new WP_Error('mlb_vimeo_api_error', $msg, ['code' => $code, 'body' => $body]);
        }
    ...
    public static function get_video_data($video_id)
    {
        ...
        if (!$video_id) return new WP_Error('mlb_vimeo_invalid_id', 'Video ID inválido.');
    ...
    public static function get_embed_html($video_id, $width = 960)
    {
        ...
        return '<div class="mlb-vimeo-embed" style="position:relative;padding-top:56.25%;border-radius:12px;overflow:hidden;">
